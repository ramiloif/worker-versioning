...
import { deliverOrder, versioningExample } from '../commit-a/workflowsV1';
...
async function run() {
  ...
  await firstWorkflow.signal(deliverOrder, 'go');
  ...
  await firstWorkflow.signal(deliverOrder, 'go');
  await secondWorkflow.signal(deliverOrder, 'go');
  await firstWorkflow.signal(deliverOrder, 'finish');
  await secondWorkflow.signal(deliverOrder, 'finish');
